...
var InvalidPokemon = "InvalidPokemon"
...
func Options(c *got.TransformContext) error {
	...
	// result.WriteString("\n// This section was generated by the `options` attribute\n")

	// result.WriteString(fmt.Sprintf("type %s interface {\n", enumName))
	// result.WriteString(fmt.Sprintf("\t__%s()\n", enumName))
	// result.WriteString("}\n\n")
	...
	// 	result.WriteString(fmt.Sprintf("type __%s string\n", k))
	// 	result.WriteString(
	// 		fmt.Sprintf("func (__%s) __%s() {}\n\n", k, enumName),
	// 	)
	// }

	// for k, v := range enumValues {
	// 	result.WriteString(fmt.Sprintf("const %s __%s = %s\n", k, k, v))
	// }
